...
def get_symbol():
    ...
    splits = sym.split(data, indices_or_sections=LENGTH // 2, axis=1)
    last0, last1 = splits[0], splits[0]
    lastH = None
    hs = []
    for i in range(LENGTH):
        last0, h = sru_node(splits[i] if i < LENGTH // 2 else lastH, last0)
        last1, lastH = sru_node(h, last1)
        if i < LENGTH // 2:
            hs.append(lastH)
    hs.append(last0)
    hs.append(last1)
...
def get_workload(batch_size, image_shape=(1, 1, EMBED_SIZE * LENGTH // 2), dtype="float32"):
